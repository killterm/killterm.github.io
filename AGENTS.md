...
- 텍스트 도구(T): 캔버스 클릭 위치에 텍스트 레이어 추가. 텍스트는 캔버스에
  베이크되지만 원본 속성(내용/크기/색)을 layer.text에 보관해 "선택 레이어에
  적용"으로 재베이크 수정이 가능하다 (재베이크도 canvas 교체라 불변 규칙 유지).
...
- 오버레이/선택 좌표는 전부 **문서 픽셀 기준**이고, 핸들·선 두께만 표시 배율로
  나눠 화면에서 일정하게 보이게 한다.
- 내보내기는 보이는 레이어 합성(flatten) → PNG/JPG/WebP. canvas 재인코딩이라
  EXIF가 제거된다(개인정보 보호 기능으로 안내).
