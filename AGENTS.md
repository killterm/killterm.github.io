...
- `tsconfig.json`의 `exclude`에 `public`을 넣어 둔다 — 벤더 파일(draco 디코더)이
  검사 대상이 되면 힌트 수십 개로 결과가 뒤덮인다.
...
- **헥스 편집기**(`HexTab.astro` + `src/lib/hex-editor.ts`):
  - 색상은 `xxd -R`와 같은 갈래로 나눈다 — 널(00) · 출력 가능 ASCII(21~7e) ·
    공백류(09~0d·20) · 0xFF · 그 밖의 제어/상위 바이트. 이 다섯으로도 헤더·문자열·
    패딩·압축 구간이 눈으로 갈린다. 색은 `light-dark()`로 두 테마 모두 잡는다.
    엔트로피 모드(줄 단위 섀넌 엔트로피)와 수정 바이트 모드를 함께 제공한다.
  - **가상 스크롤 필수**: 보이는 줄만 그린다(줄 높이 20px 고정 + 스페이서).
    16바이트/줄이면 10MB 파일이 65만 줄이라 전부 그리면 브라우저가 멈춘다.
  - 실행취소는 스냅숏이 아니라 **연산 기록**이다 — `applyEdit()`이 결과와 역연산을
    함께 돌려주므로 큰 파일에서도 메모리를 거의 쓰지 않는다. 삽입·삭제는 뒤쪽
    오프셋이 밀리므로 '수정한 바이트' 표시는 초기화한다(덮어쓰기만 추적).
  - `formatHexDump()`는 xxd 기본 출력과 **바이트 단위로 같은 형식**이다
    (오프셋 8자리 + 2바이트 묶음 8개 + 공백 2칸 + ASCII). Node 테스트가 고정
    문자열로 이를 검증하므로 형식을 바꾸면 테스트도 함께 고칠 것.
  - 파일 형식 판별은 `src/lib/polyglot.ts`의 `detectCarrier()`를 재사용한다.
    `dataEnd` 뒤에 남은 바이트를 '덧붙은 데이터'로 표시해 폴리글랏 흔적을 보여준다.
...
- 예시 시트는 `public/sprites/` — AI 제작(CC0): ball.png(48×48×8, squash
...
- **채널은 1~8개 가변**(기본 4, 상한 8 = SNES와 같은 수). `addChannel`/
  `removeChannel`은 채널 배열과 **모든 패턴의 노트 줄을 함께** 늘리고 줄인다 —
  둘이 어긋나면 재생·믹스가 깨진다. `normalizeSong`은 입력 채널 수를 1~8로
  제한하고 패턴도 같은 수로 맞춰, 옛 4채널 저장본과 8채널 곡을 모두 연다.
  믹서 행은 GlassSlider가 Astro 컴포넌트라 JS로 만들 수 없어 **8행을 미리
  렌더해 두고 남는 행을 hidden** 처리한다.
- 내장 악기는 16종이다. **목록 순서를 바꾸지 말 것** — `defaultSong()`과 옛 예시 곡
  생성 스크립트가 앞쪽 인덱스를 참조한다. 새 악기는 뒤에 덧붙인다.
- **모노포닉(스텝당 한 음), 노트는 원샷**(길이는 엔벨로프가 결정)
...
- 비우기는 세 단계다: 채널별(현재 패턴의 한 줄) · 이 패턴(모든 채널) ·
  전체(모든 패턴). 전체 비우기는 악기·볼륨·BPM·시퀀스를 남기고 노트만 지운다 —
  "새 곡"과 다른 점이다(예시 곡 편성을 유지한 채 다시 찍어보기 위함).
- **오디오 경로는 채널별 게인 → 채널별 분석 노드 → 마스터 게인 → 마스터 분석
  노드 → 출력**이다. 노트(원샷)는 자기 채널 게인에만 연결한다.
  - 볼륨·음소거·마스터를 노드가 들고 있으므로 **재생 중 조절이 즉시 반영**된다.
    게인 식은 `volume × 0.5`(헤드룸) × master로, `mixSong`의 WAV 내보내기와
    같아야 들은 것과 저장된 것이 일치한다.
  - 노드 풀은 `MAX_CHANNELS`개로 고정하고 인덱스를 재사용한다 — 채널을 지워도
    노드를 다시 만들지 않으므로 **재생 중 추가·제거가 계속 안전하다**(이미 예약된
    소리 최대 0.12초 분량만 새 채널 게인으로 울린다).
- **채널별 실시간 파형 미터**: 분석 노드의 `getByteTimeDomainData`를 행마다 작은
  캔버스에 그린다. 파형이 위아래 벽(±1)에 닿으면 캔버스 테두리가 빨개지고,
  마스터에서 닿으면 "클리핑" 표시가 뜬다 — 볼륨을 얼마나 내려야 하는지 눈으로
  판단하게 하는 것이 목적이다. 재생이 멈추고 1.5초가 지나면 rAF 루프를 쉰다.
- 예시 곡 볼륨은 **클램프 전 최대 진폭이 0.9를 넘지 않도록** 맞춰 두었다
  (스크래치패드의 재조정 스크립트로 밸런스는 유지하고 전체만 낮춤). 채널이 많고
  템포가 빠르면 원샷이 겹쳐 쉽게 1.0을 넘어 잘려 나간다(거친 왜곡).
- **지우는 동작에 `confirm()`을 쓰지 않는다.** 브라우저가 반복된 네이티브
  대화상자를 차단하면 confirm이 항상 false를 돌려줘 삭제가 조용히 무시된다
  (실제로 채널 삭제가 먹지 않는 증상으로 겪었다). 대신 곧바로 실행하고 상태 줄에
  **되돌리기** 버튼을 띄운다. 채널 삭제는 `removeChannel`이 지운 채널과 패턴별
  노트 줄을 함께 돌려주고 `restoreChannel`이 같은 자리에 되꽂는다. 노트를 새로
  찍으면 오래된 스냅숏이 남지 않도록 되돌리기 기회를 없앤다.
- **멜로디 따오기**(`src/lib/pitch-detect.ts`): 마이크 녹음이나 오디오 파일에서
  음높이를 읽어 현재 채널·패턴을 채운다. 되돌리기를 함께 제공한다.
  - **단선율만 지원한다.** 여러 악기가 섞인 완성곡의 다성 채보는 규칙 기반으로는
    쓸모없는 결과가 나오므로 시도하지 않고, 화면에도 그렇게 안내한다. 하려면 음원
    분리·채보 신경망이 필요해 서버 없는 정적 사이트와 맞지 않는다.
  - 피치 검출은 YIN 계열(차분 함수 → 누적 평균 정규화 → 첫 임계 통과 + 포물선
    보간). 자기상관만 쓰면 옥타브 아래로 잘못 잡히는데 정규화가 그걸 줄인다.
    프레임 2048/홉 512, 탐색 70~1400Hz, 중앙값 필터로 프레임 단위 튐을 없앤다.
  - **"음 없음"을 -1로 표시하지 말 것** — C3보다 낮은 음은 인덱스가 음수여서
    통째로 사라진다(실제로 그 버그를 냈다). null을 쓴다.
  - 음역이 피아노롤(C3~B5)을 벗어나면 옥타브 단위로 옮기되 **0에 가까운 이동을
    먼저** 시도해 원래 음높이를 최대한 지킨다.
  - 격자는 곡의 BPM 기준 16분음표. 격자를 넘거나 같은 칸에 겹친 음은 개수로 보고한다.
- 곡은 localStorage `killterm-composer-song`에 자동 저장 + JSON
  내보내기/가져오기(만다라트 관례). 가져오기는 `normalizeSong()`으로 형태
  검증·기본값 보강.
- **예시 곡 3개**(AI 제작 · CC0), `public/songs/`. 표기는 "자체 제작"이 아니라
  **AI 제작**으로 적는다 — 사람이 작곡한 것으로 오해되지 않게 하기 위함이다:
  - `demo.json` — "밝은 행진", C장조 128BPM 4채널(리드·베이스·킥·스네어).
  - `demo2.json` — "Am 아르페지오", 100BPM **7채널**(리드 2·베이스 2·드럼 3)로
    다채널 편성을 보여준다. 16분 아르페지오가 화음을 끌고 간다.
  - `demo3.json` — "질주", Em 168BPM 6채널. 16분 베이스 오스티나토 + 8분 리드.
    빠른 템포에 여러 채널이 겹치면 쉽게 클리핑되므로 채널 볼륨을 낮게 잡았고,
    생성 스크립트가 클립 비율 0.5% 초과를 에러로 막는다.
  - `demo4.json` — "잊혀진 곳", Dm 72BPM 6채널. 느리고 어두운 곡으로 패드·벨·톰의
    여백을 살렸다. 생성 스크립트는 악기를 인덱스가 아니라 **이름으로 찾는다**
    (`BUILTIN_INSTRUMENTS`에 악기가 추가돼도 깨지지 않게).
  둘 다 내장 악기 파라미터를 그대로 담도록 생성 스크립트에서
  `BUILTIN_INSTRUMENTS`를 임베드해 만들었다(재생성은 스크립트로). 버튼은
  `data-demo`(경로)·`data-demo-name`(표시 이름)만 주면 늘릴 수 있고, 현재 곡을
  덮어쓰므로 되돌리기를 함께 제공한다.
