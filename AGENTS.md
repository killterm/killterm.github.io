...
### 이미지 도구 (`src/pages/image.astro`)

- EXIF 확인 + 리사이즈 + PNG/JPG/WebP 변환. 이미지는 브라우저 밖으로 전송되지
  않으며(안내 문구 유지), canvas 재인코딩이라 다운로드 파일에서 EXIF가 제거된다
  (개인정보 보호 기능으로 안내).
- EXIF 파서는 `src/lib/exif.ts` — JPEG APP1의 TIFF IFD를 직접 파싱(의존성 없음),
  GPS 좌표는 sensitive로 표시해 경고를 띄운다.
- 작업 이미지는 캔버스에 보관되어 자르기/리사이즈 결과가 다음 작업의 입력이 된다.
  원본 ImageBitmap을 유지해 "원본으로 되돌리기"를 지원한다.
- 자르기: 미리보기 위 오버레이 캔버스에 드래그 선택(비율 프리셋 자유/1:1/4:3/
  16:9/9:16), 표시 좌표를 원본 픽셀로 환산해 잘라낸다. 미리보기 크기가 바뀌면
  좌표가 어긋나므로 renderPreview가 자르기 모드를 닫는다.
