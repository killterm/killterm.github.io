...
- 내장 악기 목록에는 **실제 음원 칩 채널을 흉내낸 프리셋**이 포함된다 — NES 2A03
  (펄스 12.5/25/50%·삼각 베이스·노이즈 스네어)과 게임보이 DMG(파형 베이스·펄스
  스윕·노이즈 드럼). **음색은 하드웨어의 기술적 특성이라 자유롭게 흉내낼 수 있다**
  — 특정 곡의 멜로디를 옮기는 것과는 다른 문제다.
  사각파 듀티는 `duty`로 조절한다: 0 → 50%, 0.5 → 25%, 0.75 → 12.5%. **목록 순서를 바꾸지 말 것** — `defaultSong()`과 옛 예시 곡
...
- 곡은 localStorage `killterm-composer-song`에 자동 저장 + JSON
  ...
  검증·기본값 보강.
- **예시 곡**(AI 제작 · CC0)은 `public/songs/`에 있다. 표기는 "자체 제작"이 아니라
  **AI 제작**으로 적는다 — 사람이 작곡한 것으로 오해되지 않게 하기 위함이다:
  ...
  - `demo8.json` — "긴 여정", Am 118BPM 7채널. **패턴 8개를 시퀀스 15칸으로 이은
    약 1분 곡**으로, 인트로·벌스·벌스 변형·프리코러스·코러스·코러스 변형·브레이크·
    아웃트로 구성이다. 반주(아르페지오·베이스·드럼)는 코드 진행에서 규칙적으로
    생성하고 멜로디만 구간별로 적었다 — 여덟 패턴을 손으로 다 채우면 실수가 난다.
    생성 스크립트는 패턴들이 서로 충분히 다른지(노트 수 구성)까지 검사한다.
  - `demo5~7.json` — 8비트 시대 분위기의 창작곡 3개. demo5 "달빛 유영"(Em 92BPM,
    NES식 2채널 에코 — 같은 멜로디를 2스텝 늦춰 작은 볼륨으로 깐다), demo6
    "초원 산책"(G 148BPM, 게임보이 듀티 리드 + 파형 베이스), demo7 "최종 결전"
    (Bm 하모닉 마이너 176BPM 7채널).
- 예시 곡들은 내장 악기 파라미터를 그대로 담도록 생성 스크립트에서
...
### 폰트 만들기 (`src/pages/font.astro`)

- 칸을 칠해 글자를 그리고 **설치 가능한 .ttf로 내보낸다.** 의존성 없이 바이너리
  테이블을 직접 쓴다(`src/lib/ttf-encode.ts`).
- **글리프 모델이 두 편집 방식을 담는다**(`src/lib/bitmap-font.ts`):
  `{kind:'pixel', bits}` 또는 `{kind:'outline', contours}`. `glyphToContours()`가
  어느 쪽이든 컨투어로 바꾸고 TTF 라이터는 컨투어만 본다 — 벡터(베지어) 편집을
  나중에 얹어도 내보내기 경로를 고칠 필요가 없다. 점에 `onCurve` 플래그가 이미
  있어 2차 베지어도 그대로 표현된다.
- 픽셀은 **직사각형으로 병합**해 컨투어로 만든다(`pixelsToRects`, 그리디). 한 픽셀
  = 사각형 하나로 두면 점이 네 배로 늘어난다. 픽셀 좌표는 위→아래, 폰트 좌표는
  아래→위라 y를 뒤집고 baselineRow 기준으로 원점을 맞춘다.
- `unitsPerEm = grid × 64`(16×16 → 1024). 픽셀 한 칸이 정확히 64 단위라 좌표가
  모두 정수로 떨어진다.
- TTF 테이블: `head hhea maxp hmtx cmap glyf loca name OS/2 post`. 글리프 0은
  `.notdef`, `cmap`은 format 4(BMP 전체 — 한글 음절도 담긴다), `loca`는 long
  형식, `post`는 3.0. **체크섬은 테이블별 + `checkSumAdjustment`까지** 계산해야
  폰트 스택이 받아들인다(전체 체크섬 + adjustment = 0xB1B0AFBA).
- **한글은 자모만 그려 조합한다**(`src/lib/hangul-compose.ts`). 음절 11,172자를
  손으로 그리는 건 불가능하므로 초성 19·중성 21·종성 27자를 그려 두고 조합한다.
  중성 모양(세로형 ㅏ / 가로형 ㅗ / 복합형 ㅘ)과 받침 유무로 배치 사각형이 정해지며,
  배치는 0~1 비율로 표현해 그리드 크기와 무관하다.
- **자모는 자리에 따라 모양이 달라야 한다 — 그래서 "벌"을 나눈다.** 초성 ㄱ은
  '가'·'고'·'각'에서 폭·높이·위치가 다르고 받침으로 갈 때 또 다르다. 벌 구성은
  초성 6벌(중성 3그룹 × 받침 유무) · 중성 2벌(받침 유무) · 종성 3벌(위 모음 모양).
  - **벌이 그려져 있으면 변형 없이 OR로 겹친다** — 작성자가 의도한 획 두께와 위치가
    유지된다. 벌 비트맵은 전각 칸에 최종 위치대로 그린다.
  - 벌이 없으면 **기본 벌(0)을 배치 사각형에 맞춰 늘려 찍는다**. 그래서 자모 67자만
    그려도 바로 글자가 나오고, 벌을 채우면 품질이 올라가는 점진적 구조다.
  - 조합은 `JamoLookup`(종류·인덱스·벌 → 비트맵) 함수로 추상화했다. 페이지는 자모를
    `종류:인덱스:벌` 키 맵으로 보관하고, 기본 벌만 있는 경우는 `baseLookup()`을 쓴다.
  - 표준 조합형은 초성 8벌(ㄱ·ㅋ을 따로 둠)·중성 4벌까지 나누는데, 여기서는 6/2/3으로
    줄였다. 더 정교하게 가려면 이 갈래를 늘리면 된다.
- **벡터(아웃라인) 편집**은 같은 페이지의 두 번째 모드다. 글자마다 픽셀/벡터를
  고르고, "픽셀 → 벡터로 변환"(`pixelGlyphToOutline`)으로 지금 그림을 사각 컨투어로
  승격시켜 다듬는 흐름을 기본으로 삼았다 — 빈 화면에서 베지어를 찍는 것보다 쉽다.
  - 도구는 점 이동·추가·삭제·곡선 전환(on/off-curve 토글) + 새 윤곽선 · 방향 반전.
    좌표는 폰트 단위로 다루고 "격자에 맞추기"는 반 칸(32 단위)에 붙인다(`snapUnits`).
  - 캔버스 렌더는 `contourToPath()`로 얻은 명령을 그대로 Canvas 경로에 흘린다.
    **연속된 off-curve 사이에는 중점이 암시적 on-curve 점으로 들어간다**(TrueType 규칙)
    — 이 규칙을 라이브러리 한 곳에 두어 미리보기와 내보내기가 어긋나지 않게 했다.
  - 구멍(ㅇ·o)은 **안쪽 고리의 진행 방향을 반대로** 두어 만든다. 폰트 단위는 위가 +y라
    시계 방향(부호 면적 < 0)이 채움, 반시계가 구멍이다(`contourSignedArea`).
    `reverseContour`는 **시작점을 두고 나머지만 뒤집어** 두 번 적용하면 원래 배열로
    돌아온다(실행취소·비교가 단순해진다).
  - 벡터 → 픽셀 자동 역변환은 넣지 않았다(래스터화하면 곡선이 사라진다). 되돌리려면
    실행취소를 쓴다. 실행취소 기록도 `bits`가 아니라 **글리프 전체를 복제**해 담는다.
  - `glyf` 좌표는 정수라야 하므로 라이터가 쓰기 직전에 반올림한다. 반올림을 앞단에
    두면 소수 좌표가 델타 누적에서 어긋나 모양이 흐트러진다.
  - **자모는 픽셀 전용이다.** 조합이 비트맵 OR·확대 연산이라 윤곽선으로는 같은 방식을
    쓸 수 없다. 모드 선택 줄 자체를 자모 탭에서 숨긴다.
  - 그리드 크기를 바꾸면 픽셀은 `resizePixels`, 윤곽선은 **좌표를 같은 비율로 곱한다**
    — `unitsPerEm`이 그리드에 비례하므로 비율을 맞춰야 크기가 유지된다.
- **SVG 불러오기**(`src/lib/svg-path.ts` + `src/lib/svg-import.ts`)로 밖에서 그린 모양을
  글리프로 가져온다. 어려운 부분은 두 가지다.
  - **SVG는 3차 베지어(C)·원호(A), TrueType은 2차 베지어만 담는다.** 3차 한 개를 2차
    한 개로 줄일 때 제어점은 `(3c1 + 3c2 − p0 − p3)/4`이고 최대 오차는
    `|p3 − 3c2 + 3c1 − p0| · √3/36`이다(수치 실험에서 정확히 일치). 허용치를 넘으면
    de Casteljau로 반씩 쪼갠다 — 오차가 분할마다 1/8로 줄어 몇 번이면 충분하다.
    허용치는 `unitsPerEm / 500`(em의 0.2%)으로 두었다. 원호는 규격 F.6.5의
    endpoint → center 파라미터화로 90° 이하 3차 조각을 거쳐 들어온다.
  - **채움/구멍은 중첩 깊이로 판정한다.** 컨투어를 펼쳐 point-in-polygon으로 깊이를
    세고 짝수는 시계(채움), 홀수는 반시계(구멍)로 맞춘다. 원본이 nonzero든 evenodd든,
    그린 방향이 어느 쪽이든 같은 결과가 나와 'O'·'ㅇ'의 구멍이 저절로 뚫린다.
  - **y 뒤집기와 방향 정리의 순서가 중요하다.** 방향을 먼저 정하고 y를 뒤집으면 부호가
    반대가 되어 채움과 구멍이 뒤바뀐다. 그래서 `collectSvgContours()`가 맞춤(뒤집기 +
    스케일)까지 하고 **그 뒤에** `normalizeWinding()`을 부른다 — 호출자가 순서를 틀릴
    여지를 없앴다.
  - **순수 모듈과 DOM 층을 나눴다.** 경로 파싱·베지어 변환·방향 판정은 `svg-path.ts`에
    두어 Node에서 그대로 테스트하고, `DOMParser`로 트리를 걷고 `transform`을 합성하는
    얇은 층만 `svg-import.ts`에 있다(테스트에서는 parse5로 DOM을 흉내 낸다).
  - 도형(`rect`·`circle`·`ellipse`·`line`·`polyline`·`polygon`)은 `d`로 바꿔 같은 길로
    보낸다. 아이콘 SVG가 path만으로 되어 있는 경우는 드물다.
  - **담지 못한 것은 조용히 버리지 않고 경고로 모아 돌려준다**(`<text>`·`<image>`·
    `fill="none"`·읽지 못한 transform). 글자가 사라진 이유를 알 수 없으면 도구를
    의심하게 된다. 획(stroke)의 외곽선화는 범위 밖이라 안내만 한다.
  - 크기 맞춤은 프리셋 두 개(전각 칸 = 그리드 전체에 가운데 / 대문자 높이 = 밑변을
    베이스라인에)뿐이고, 나머지는 기존 벡터 도구로 다듬는다. **자모는 프리셋과 무관하게
    전각 칸**에 맞춘다(조합이 칸 전체를 쓴다) — 그리고 픽셀 전용이라 `rasterizeOutline`으로
    구워 넣는다.
  - 파일 하나는 현재 글자, 여러 개는 파일명(`A.svg` · `U+AC00.svg`)을 글자로 본다.
    일괄 가져오기는 글리프별 실행취소로는 번거로우니 프로젝트 스냅숏 되돌리기를 쓴다.
  - 벡터 글리프의 기본 폭은 윤곽선 경계에서 구한다 — 픽셀 경로만 보던 `defaultAdvance()`는
    SVG를 불러오면 폭이 그림과 무관하게 나왔다.
- 내보내기 세 가지: TTF · BDF(터미널·임베디드, 픽셀 그대로) · PNG 아틀라스 +
  JSON 메트릭(게임 엔진용). 한글 범위는 미리보기 문장의 음절 / 조합 가능한 음절
  전부(상한 3,000) / 없이 중에서 고른다 — 자모를 많이 그리면 수천 자가 되어
  파일이 커진다.
- BDF·PNG 아틀라스는 비트맵 형식이라 벡터 글리프를 담을 수 없다. 그냥 빼면 사용자는
  글자가 사라진 이유를 알 수 없으므로, **오프스크린 캔버스로 grid×grid로 다시 구워**
  담는다(`rasterizeOutline`, 알파 > 127을 켜진 픽셀로 본다). TTF만 곡선을 살린다.
- 미리보기는 만든 TTF를 `FontFace`로 등록해 실제 렌더링을 보여준다(이미지 편집기의
  폰트 첨부와 같은 방식). 그리는 동안 매번 만들지 않도록 200ms 미룬다.
- **검증은 자체 파서와 시스템 도구를 함께 쓴다.** Node 테스트가 테이블 디렉터리·
  체크섬·cmap 조회·glyf 경계를 되읽고, `fc-query`/`fc-validate`로 실제 폰트 스택이
  파싱하는지와 문자 커버리지까지 확인한다(자체 파서만으로는 놓치는 형식 오류가 있다).
  곡선은 한 걸음 더 나아가 `hb-view`로 실제 래스터 이미지를 뽑아 확인했다 — off-curve
  점이 살아 있고 구멍이 뚫리는지는 렌더링해 봐야 확실하다. SVG 불러오기도 같은 방식으로
  확인한다(구멍 있는 'a' + Q·T 곡선 도형을 통과시켜 렌더).
- 3차 → 2차 근사를 테스트할 때는 **샘플 간격이 오차로 잡히지 않게** 촘촘히 뽑아야 한다.
  조각마다 30점만 뽑으면 간격 자체가 허용치를 넘어 멀쩡한 근사가 실패로 나온다.
- 작업은 localStorage `killterm-font-project`에 자동 저장 + 프로젝트 JSON
  내보내기/가져오기. 지우는 동작은 confirm 없이 되돌리기를 제공한다(작곡기와 같은 이유).
