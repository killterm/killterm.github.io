...
- **재생 중에도 채널을 제거·추가할 수 있다.** 스케줄러가 스텝마다 `song.channels`를
  다시 읽고 노트는 채널에 묶이지 않은 원샷이라, 인덱스가 밀려도 이미 예약된 소리
  (최대 0.12초 앞)만 울리고 끝난다. 채널별 게인 노드를 두지 않은 덕이다 —
  루퍼(루프 소스를 계속 재생)와 달리 여기서는 살아 있는 게인 갱신이 필요 없다.
